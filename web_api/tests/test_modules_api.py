...
def test_claim_session_success():
    ...
    with patch("web_api.routes.modules.get_current_user") as mock_auth:
        ...
        with patch("web_api.routes.modules.get_or_create_user") as mock_get_user:
...
def test_claim_already_claimed_session():
    ...
    with patch("web_api.routes.modules.get_current_user") as mock_auth:
        ...
        with patch("web_api.routes.modules.get_or_create_user") as mock_get_user:
            ...
            with patch("web_api.routes.modules.claim_session") as mock_claim:
                from core.modules import SessionAlreadyClaimedError
...
def test_claim_nonexistent_session():
    ...
    with patch("web_api.routes.modules.get_current_user") as mock_auth:
        ...
        with patch("web_api.routes.modules.get_or_create_user") as mock_get_user:
            ...
            with patch("web_api.routes.modules.claim_session") as mock_claim:
                from core.modules import SessionNotFoundError
...
def test_get_anonymous_session_by_id():
    ...
    with patch("web_api.routes.modules.get_optional_user") as mock_auth:
        mock_auth.return_value = None  # Not authenticated

        with patch("web_api.routes.modules.get_session") as mock_get:
            mock_get.return_value = {
                "session_id": 1,
                "user_id": None,  # Anonymous
                "module_slug": "intro-to-ai-safety",
                "current_stage_index": 0,
                "messages": [],
                "completed_at": None,
            }

            with patch("web_api.routes.modules.load_module") as mock_module:
                ...
                mock_module_obj.stages = [mock_stage]
                mock_module.return_value = mock_module_obj

                with patch("web_api.routes.modules.get_stage_content") as mock_content:
                    mock_content.return_value = None

                    response = client.get("/api/module-sessions/1")
...
def test_create_anonymous_session():
    ...
    with patch("web_api.routes.modules.get_optional_user") as mock_auth:
        ...
        with patch("web_api.routes.modules.create_session") as mock_create:
            ...
            with patch("web_api.routes.modules.load_module") as mock_module:
                ...
                mock_module_obj = MagicMock()
                mock_module_obj.stages = []  # No stages = no started message
                mock_module.return_value = mock_module_obj

                response = client.post(
                    "/api/module-sessions", json={"module_slug": "intro-to-ai-safety"}
                )
        ...
        with patch("web_api.routes.modules.get_or_create_user") as mock_get_user:
            ...
            with patch("web_api.routes.modules.create_session") as mock_create:
                ...
                with patch("web_api.routes.modules.load_module") as mock_module:
                    ...
                    mock_module_obj = MagicMock()
                    mock_module_obj.stages = []
                    mock_module.return_value = mock_module_obj

                    response = client.post(
                        "/api/module-sessions",
                        json={"module_slug": "intro-to-ai-safety"},
                    )
