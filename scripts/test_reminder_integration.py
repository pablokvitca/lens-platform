...
async def main(email: str, delay_seconds: int = 60):
    ...
    from core.tables import meetings, groups, users, groups_users
    ...
    async with get_connection() as conn:
        ...
        user_result = await conn.execute(
            select(users.c.user_id, users.c.email, users.c.nickname).where(
                users.c.email == email
            )
        )
        ...
        print(
            f"   Found user: {user['nickname'] or user['email']} (id={user['user_id']})"
        )

        # Find any active group the user is in
        group_result = await conn.execute(
            select(
                groups.c.group_id, groups.c.group_name, groups.c.discord_text_channel_id
            )
        ...
        if not group:
            print("   ERROR: User is not in any active group")
            shutdown_scheduler()
            return
        ...
        if meeting:
            print(
                f"   Using existing meeting: id={meeting['meeting_id']}, scheduled={meeting['scheduled_at']}"
            )
    ...
    print("   Reminder type: reminder_24h (test)")
    print(
        f"   Will fire at: {run_at.strftime('%H:%M:%S')} (in {delay_seconds} seconds)"
    )
    ...
    print("   Watch for notification logs below...\n")
    print("-" * 60)
    import sys
    ...
    print("   Also check the logs above for any errors.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Test APScheduler reminder integration"
    )
    parser.add_argument(
        "--email", required=True, help="Your email address in the local DB"
    )
    parser.add_argument(
        "--delay",
        type=int,
        default=60,
        help="Seconds until reminder fires (default: 60)",
    )
