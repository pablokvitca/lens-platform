...
def truncate_after(
    file_path: str, ending_phrase: str, dry_run: bool = False
) -> tuple[bool, str]:
    """
    ...
    if len(matches) > 1:
        ...
        return (
            False,
            f"Phrase appears {len(matches)} times (lines {', '.join(map(str, line_nums))}). Use a more unique phrase.",
        )
    ...
    if dry_run:
        ...
        return (
            True,
            f"Would remove {removed_lines} lines ({removed_chars} chars) after phrase.\nPreview of removed content:\n{preview}",
        )
    ...
    return (
        True,
        f"Removed {removed_lines} lines ({removed_chars} chars) after '{ending_phrase}'",
    )
...
def main():
    if len(sys.argv) < 3:
        ...
        print(
            'Usage: python truncate_after.py <file_path> "<ending_phrase>" [--dry-run]'
        )
        sys.exit(1)
