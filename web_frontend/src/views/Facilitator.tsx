...
export default function Facilitator() {
  ...
  const handleChatClick = useCallback(
    (userId: number, userName: string, moduleSlug: string, moduleTitle: string) => {
...
function VerticalTimeline({
  ...
  onChatClick: (userId: number, userName: string, moduleSlug: string, moduleTitle: string) => void;
            ...
            return (
              <div key={tm.user_id} className="shrink-0">
                ...
                {segs.map((seg, si) => {
                  ...
                  const modChat = seg.slug ? moduleStats[seg.slug]?.chat_count ?? 0 : 0;
                  const isActiveChat = selectedChat && selectedChat.userId === tm.user_id && selectedChat.moduleSlug === seg.slug;
                  const chatColor = isActiveChat ? "text-indigo-600" : "text-indigo-400";
                  const bracketBorder = isActiveChat ? "border-indigo-500" : "border-indigo-300/60";
                  return (
                    <div
                      key={si}
