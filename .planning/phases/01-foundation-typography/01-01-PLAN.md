...
must_haves:
  truths:
    ...
    - "Body text is optimized at 18px base with 1.6 line height for comfortable reading"
  artifacts:
    ...
    - path: "web_frontend/src/styles/globals.css"
      provides: "Safe area CSS vars, touch-action, body text (18px/1.6), typography scale"
      contains: ["--safe-top", "touch-action: manipulation", "font-size: 18px", "line-height: 1.6", "h1", "h2", "h3"]
  key_links:
...
<task type="auto">
  ...
  <action>
    ...
    3. Body text optimization (add to body selector):
    ```css
    body {
      font-size: 18px;
      line-height: 1.6;
    ...
    ```
    This provides comfortable reading on mobile. 18px exceeds iOS's 16px input zoom threshold.

    4. Mobile typography scale (add to @layer base):
    ...
    Note on safe area variables: The CSS custom properties (--safe-top, --safe-bottom, etc.)
    are defined here as infrastructure. They will be applied to layout components in Phase 2
    (Navigation/Layout) when those components are refactored for mobile. This phase establishes
    the variables; Phase 2 consumes them.
  </action>
  <verify>
    ...
    - `grep "font-size: 18px" web_frontend/src/styles/globals.css` returns match (body text)
    - `grep "line-height: 1.6" web_frontend/src/styles/globals.css` returns match (body text)
  ...
  <done>
    Viewport meta includes viewport-fit=cover. globals.css has safe area variables, touch-action, text-size-adjust, body text styles (18px/1.6), and mobile typography scale. Build passes.
  </done>
</task>
...
- CSS custom properties (--safe-top, etc.) ready for safe area padding in Phase 2 (Navigation/Layout)
- Body text optimized at 18px base, 1.6 line height
